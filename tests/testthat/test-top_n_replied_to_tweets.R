...
test_that("result is a ggplot2 object", {
  expect_true(ggplot2::is.ggplot(top_n_replied_to_tweets(sqlite_con, n = 10)))
})


test_that("ggplot2 plot has expected output", {
  vdiffr::expect_doppelganger("top_n_replied_to_tweets_10",
                              top_n_replied_to_tweets(sqlite_con, n = 10))
})
...
test_that("ggplot2 plot has expected output (excl RT)", {
  vdiffr::expect_doppelganger("top_n_replied_to_tweets_10_excl_RT",
                              top_n_replied_to_tweets(sqlite_con,
                                                      n = 10,
                                                      exclude_RT = TRUE))
})
