...
test_that("list of length 2 is created as expected (hourly plot)", {
  results_hourly <- num_tweets_by_timeperiod(sqlite_con, "hour",
                                             return_data = TRUE)
...
test_that("list of length 2 is created as expected (daily plot)", {
  results_daily <- num_tweets_by_timeperiod(sqlite_con, "day",
                                            return_data = TRUE)
...
test_that("second element of list (data) is a tibble (daily plot)", {
  results_daily <- num_tweets_by_timeperiod(sqlite_con, "day",
                                            return_data = TRUE)
  expect_true(tibble::is_tibble(results_daily$data))
})
...
test_that("list of length 2 is created as expected (monthly plot)", {
  results_monthly <- num_tweets_by_timeperiod(sqlite_con, "month",
                                              return_data = TRUE)
